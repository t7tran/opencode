...
import { SEALED_PREFIX, isSealed, keyFingerprint, seal, unseal, unwrapKey } from "../../src/fork/key-seal"
import { managedKey, managedKeyActive } from "../../src/fork/key-file"
import { lockedManagedEntry, lockedProviderManaged } from "../../src/fork/lock"
import { pepperFilePath, readPepperFile } from "../../src/fork/pepper"
...
describe("fork.key-file with a sealed file", () => {
  ...
  test("the locked provider picks up the unsealed key", () => {
    ...
    expect(lockedManagedEntry()?.[1].options?.apiKey).toBe(KEY)
  })
