...
//   3. lockedManagedSettings() pins both the key and the gateway URL, and is
//      absent when there is no managed key file.
...
describe("fork.lock", () => {
  test("lockedProvider returns the hardcoded Genix identity", () => {
    ...
    expect(p.npm).toBe("@opencode/ai/providers/openai-compatible")
  })
})
