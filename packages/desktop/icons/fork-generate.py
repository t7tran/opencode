...
Pass --missing-only to write just the files that do not exist yet. Use it when a
new upstream consumer needs a size the fork does not generate: it adds that file
without re-encoding the rest, which would otherwise churn every icon in the diff
on a different Pillow version. It also skips the icns step when icon.icns is
already present, so the flag works on a machine without png2icns.
"""
...
# Upstream ships full iOS and Android icon sets per channel, which this fork does
# not reproduce — nothing here builds a mobile app. These two are the exception:
# packages/app/vite.icons.ts reads them at Vite config load to emit the web UI's
# apple-touch-icon (180px) and its 192px PWA manifest icon, so the web build —
# and therefore the CLI build that bundles it — fails without them.
NESTED_SIZES = {
    os.path.join("ios", "AppIcon-60x60@3x.png"): 180,
    os.path.join("android", "mipmap-xxxhdpi", "ic_launcher.png"): 192,
}
...
def write(channel: str, source: Image.Image, missing_only: bool = False) -> None:
    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), channel)
    os.makedirs(out, exist_ok=True)

    def wanted(path: str) -> bool:
        return not (missing_only and os.path.exists(path))

    for name, size in {**SIZES, **NESTED_SIZES}.items():
    ...
    if wanted(ico):
        source.save(ico, sizes=[(s, s) for s in ICO_SIZES])

    if not wanted(os.path.join(out, "icon.icns")):
...
def main() -> int:
    ...
    needs_icns = any(
        not os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)), channel, "icon.icns"))
        for channel in CHANNELS
    )
    if (not missing_only or needs_icns) and shutil.which("png2icns") is None:
        print("png2icns not found: install icnsutils", file=sys.stderr)
        return 1
    for channel in CHANNELS:
        write(channel, render(channel), missing_only=missing_only)
    return 0
