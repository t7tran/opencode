...
export function forkManagedKey(): boolean {
  ...
  // Upstream v2 renamed the context bridge from `window.api` to `window.electron`.
  // Read defensively: this module is shared with the web app, which has neither.
  return (window as { electron?: { forkManagedKey?: boolean } }).electron?.forkManagedKey === true
}
 ...
 * which packages/cli/src/server-process.ts disables.
 */
