...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // 4. `.opencode` directories, `~/.genixcode` and OPENCODE_CONFIG_DIR
  // discovered while walking up the tree. Also returned below so callers can
  // install plugin dependencies from each location.
  const dirs = unique(directories).filter((dir) => ConfigPaths.isConfigDirectory(dir)) // fork_change

  for (const dir of dirs) {
    if (!ConfigPaths.isConfigDirectory(dir)) continue // fork_change
